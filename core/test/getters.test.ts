...
import { Sides, supportedSymbols, supportedTimeFrames } from '../store/types';
...
it('Should return the count of active orders', async () => {
    ...
    await trader.buyAtMarket(1);
    // active orders
    await trader.reducePositionAt(
        1,
        store.getState().mainReducer.currentPrice + 10,
        Sides.SELL
    );
    await trader.reducePositionAt(
        0.5,
        store.getState().mainReducer.currentPrice + 15,
        Sides.SELL
    );
...
it('Should return all the trading candles', () => {
    ...
    expect(selectors.getTradingCandles()).toEqual([
        tradingCandle1,
        tradingCandle2
    ]);
});
...
it('Should return all candles for a specific symbol and/or a specific timeFrame', () => {
    ...
    expect(
        selectors.getCandlesFor(
            supportedSymbols.LTCUSD,
            supportedTimeFrames.fiveMinutes
        )
    ).toEqual([candle3, candle4]);
});
...
it('Should return past candle', () => {
    ...
    const candle3: Candle = new Candle({
        symbol: 'ETHUSD',
        timeFrame: '5m',
        timestamp: 1543387260000 + 60000 * 2,
        open: 117.31,
        close: 127.27,
        high: 127.31,
        low: 117.17,
    ...
    const candle4: Candle = new Candle({
        symbol: 'ETHUSD',
        timeFrame: '5m',
        timestamp: 1543387260000 + 60000 * 3,
        open: 117.31,
        close: 127.27,
        high: 127.31,
        low: 117.17,
...
it('Should return past candle', () => {
    ...
    expect(
        selectors.getPastCandleFor(
            0,
            supportedSymbols.ETHUSD,
            supportedTimeFrames.fiveMinutes
        )
    ).toEqual(candle4);
    expect(
